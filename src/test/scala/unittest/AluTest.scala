...
class TestAluVector(c: AluVector) extends PeekPokeTester(c) {
  ...
  def aluRef(opcode: Int, a: Array[Int], b: Array[Int], width: Int) : Array[Int] = {
    ...
    if (opcode == 1) { //tensor maxpool
    ...
    } else if (opcode == 3) { //tensor shift
      for (i <- 0 until size) {
        res(i) = a(i) >> (b(i) & mask).toInt
      }
    } else if (opcode == 4) { //tensor leftshift 在ISA中并没有看到 leftshift 哦
      ...
      // default
      for (i <- 0 until size) { //tensor minpool
  ...
  for (i <- 0 until num_ops) {
    ...
    if(debug) {
      println("size: " + dataGen.any.size)
      for ( j <- 0 until dataGen.any.size ) {
          println("dataGen(" + j + ") " + dataGen.any(j))
      }
    }
    val op = i
  ...
  //lijun
  if (debug) {
      val printinfo = c.name + ' ' +  c.blockOut + ' '
      val aluReg =  c.f
      val ports = c.getPorts
      println("hello")
      println(printinfo)
      println("aluReg: " +  aluReg)
      println("ports: " + ports)
  }
}
